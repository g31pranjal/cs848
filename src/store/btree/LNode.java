...
class LNode<tKey extends Comparable<tKey>, tValue> extends Node<tKey> {
	...
	@SuppressWarnings("unchecked")
	public List<tValue> getValues(int index, int direction) {
		tKey key = (tKey)this.keys[index];
		...
		if(this.leftSibling != null && direction <= 0) {
			...
			if(i != -1) 
				l.addAll(((LNode<tKey, tValue>)this.leftSibling).getValues(i, -1));
		}
		l.addAll(ret);
		if(this.rightSibling != null && direction >= 0) {
			int i = this.rightSibling.search(key);
			if(i != -1) 
				l.addAll(((LNode<tKey, tValue>)this.rightSibling).getValues(i, 1));
		}
		return l;
	}
